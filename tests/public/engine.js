...
EventEmitter.prototype.on = function (name, fn) {
  ...
  if (!this.$events[name]) {
    this.$events[name] = fn;
  } else if (util.isArray(this.$events[name])) {
...
EventEmitter.prototype.listeners = function (name) {
  ...
  if (!util.isArray(this.$events[name])) {
...
});require.register("engine.io-client.js", function(module, exports, require){
...
// TODO: clarify what's this for?
var pollingIndex = [];

JSONPPolling.prototype.setIndex = function () {
  // if we have an index already, set it to empy
  if (undefined != this.index) {
    pollingIndex[this.index] = empty;
  }

  var self = this;
  this.index = pollingIndex.length;
  pollingIndex.push(function (msg) {
    self.onData(msg);
  });
};
...
XHRPolling.prototype.request = function (opts) {
  opts.uri = this.uri();
  opts.xd = this.xd;
...
XHRPolling.prototype.write = function (data, fn) {
...
FlashWS.prototype.doOpen = function () {
  ...
  var base = eio.enginePath + '/support/web-socket-js/'
    , self = this
