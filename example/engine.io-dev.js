...
Polling.prototype.pause = function (onPause) {
  ...
  function pause () {
window.console && console.warn && console.warn("debug  : paused");
  ...
  if (this.polling || !this.writable) {
    ...
    if (this.polling) {
window.console && console.warn && console.warn("debug  : we are currently polling - waiting to pause");
      total++;
      this.once('poll', function () {
window.console && console.warn && console.warn("debug  : pre-pause polling complete");
        --total || pause();
      });
    }

    if (!this.writable) {
window.console && console.warn && console.warn("debug  : we are currently writing - waiting to pause");
      total++;
      this.once('drain', function () {
window.console && console.warn && console.warn("debug  : pre-pause writing complete");
        --total || pause();
      });
    }
  } else {
...
window.console && console.warn && console.warn("debug  : polling");
  this.polling = true;
  this.doPoll();
};
...
window.console && console.warn && console.warn("debug  : polling got", data);
  // decode payload
...
window.console && console.warn && console.warn("debug  : ignoring poll - transport state \"%s\"", this.readyState);
  }
};
...
window.console && console.warn && console.warn("debug  : sending close packet");
...
window.console && console.warn && console.warn("debug  : xhr poll");
  ...
  if (this.xd && global.XDomainRequest && xhr instanceof XDomainRequest) {
    ...
    xhr.onreadystatechange = function () {
      ...
      try {
        ...
        if (200 == xhr.status || 1223 == xhr.status || 204 == xhr.status) {
...
window.console && console.warn && console.warn("debug  : sending xhr with url %s | data %s", this.uri, this.data);
  xhr.send(this.data);
...
window.console && console.warn && console.warn("debug  : creating transport \"%s\"", name);
...
Socket.prototype.setTransport = function (transport) {
  ...
  if (this.transport) {
window.console && console.warn && console.warn("debug  : clearing existing transport");
...
window.console && console.warn && console.warn("debug  : probing transport \"%s\"", name);
  var transport = this.createTransport(name, { probe: 1 })
    , self = this

  transport.once('open', function () {
window.console && console.warn && console.warn("debug  : probe transport \"%s\" opened - pinging", name);
    transport.send([{ type: 'ping', data: 'probe' }]);
    transport.once('packet', function (msg) {
      if ('pong' == msg.type && 'probe' == msg.data) {
window.console && console.warn && console.warn("debug  : probe transport \"%s\" pong - upgrading", name);
        self.upgrading = true;
        self.emit('upgrading', transport);

window.console && console.warn && console.warn("debug  : pausing current transport \"%s\"", self.transport.name);
        self.transport.pause(function () {
          if ('closed' == self.readyState || 'closing' == self.readyState) return;
window.console && console.warn && console.warn("debug  : changing transport and sending upgrade packet");
...
window.console && console.warn && console.warn("debug  : probe transport \"%s\" failed", name);
  ...
  this.once('close', function () {
    if (transport) {
window.console && console.warn && console.warn("debug  : socket closed prematurely - aborting probe");
      transport.close();
      transport = null;
    }
  });
...
window.console && console.warn && console.warn("debug  : \"%s\" works - aborting \"%s\"", to.name, transport.name);
      transport.close();
      transport = null;
    }
  });
...
window.console && console.warn && console.warn("debug  : socket open");
  this.readyState = 'open';
  this.emit('open');
...
window.console && console.warn && console.warn("debug  : starting upgrade probes");
...
Socket.prototype.onPacket = function (packet) {
  if ('opening' == this.readyState || 'open' == this.readyState) {
window.console && console.warn && console.warn("debug  : socket receive: type \"%s\" | data \"%s\"", packet.type, packet.data);
    switch (packet.type) {
      case 'open':
...
window.console && console.warn && console.warn("debug  : packet received with socket readyState \"%s\"", this.readyState);
  }
};
...
window.console && console.warn && console.warn("debug  : flushing %d packets in socket", this.writeBuffer.length);
...
window.console && console.warn && console.warn("debug  : socket closing - telling transport to close");
...
window.console && console.warn && console.warn("debug  : socket close: \"%s\"", reason);
    this.readyState = 'closed';
...
var packetslist = []; for (var i in packets) packetslist.push(i);
